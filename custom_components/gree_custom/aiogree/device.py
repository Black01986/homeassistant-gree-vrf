...
class GreeDevice:
        ...
        for transport, mac_controller, version in attempts:
            ...
            try:
                await self._client.bind(
                    ...
                    use_preferred_key_directly=isinstance(transport, GreeMqttTransport),
                )
    ...
    async def push_device_status(self) -> None:
        ...
        # Standard Gree devices expect beeper properties alongside commands.
        # VRF child devices controlled through MQTT do not: send only the
        # properties that were actually changed.
        is_vrf_mqtt = (
            isinstance(self.transport, GreeMqttTransport)
            and self.mac_address_controller != self._mac_addr
        )
    ...
    def horizontal_swing_mode(self) -> HorizontalSwingMode:
        ...
        try:
            return HorizontalSwingMode(value)
        except ValueError:
            _LOGGER.warning(
                "Unknown horizontal swing mode %s; using default", value
            )
            return HorizontalSwingMode.default
